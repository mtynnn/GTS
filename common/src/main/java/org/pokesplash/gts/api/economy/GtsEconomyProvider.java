...
public abstract class GtsEconomyProvider {
    ...
    public static GtsEconomy getHighestEconomy() {
        if (providers.get(Priority.HIGHEST) != null) return providers.get(Priority.HIGHEST);
        if (providers.get(Priority.HIGH) != null)    return providers.get(Priority.HIGH);
        if (providers.get(Priority.MEDIUM) != null)  return providers.get(Priority.MEDIUM);
        if (providers.get(Priority.LOW) != null)     return providers.get(Priority.LOW);
        if (providers.get(Priority.LOWEST) != null)  return providers.get(Priority.LOWEST);
